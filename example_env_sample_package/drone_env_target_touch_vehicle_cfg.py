...
class DroneTargetTouchVehicleBaseEnvCfg(DirectRLEnvCfg):
    """Vehicle 系列 touch 共同基礎設定（供 Example Env 繼承）。

    說明：
    - 這是新系列（Vehicle）共同基底，不依賴舊系列課程切換。
    - Example Env 只保留單一範例設定。
...
@configclass
class DroneTargetTouchVehicleExampleEnvCfg(DroneTargetTouchVehicleBaseEnvCfg):
    """Vehicle touch Example Env：目標距離範圍為 [30, 60] 公尺。"""
    ...
    # Example Env 提高控制命令懲罰，抑制大幅晃動與急劇動作切換。
    tcmd_lambda_4 = 1e-2
    tcmd_lambda_5 = 1e-3
    # Example Env 強化姿態控制：啟用角速度懲罰與前進傾角軟式引導。
